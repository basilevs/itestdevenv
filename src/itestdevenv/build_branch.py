...
def build_branch(branch=None, docker=True, installers=True, test=True, coverage=False):
    if not branch:
    ...
    build_url = jenkins_build_branch(jenkins, branch, docker=docker, installers=installers, test=test, version=version, coverage=coverage)
    print(build_url)
